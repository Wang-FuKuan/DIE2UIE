...
class RAMLora(nn.Module):
    ...
    def generate_tag_change(self,
                 image,
                 threshold=0.68,
                 tag_input=None,
                 random_k: int = 0
                 ):
        # 随机模式：直接从标签库抽样并返回（跳过视觉前向）
        if random_k and random_k > 0:
            bs = image.shape[0] if isinstance(image, torch.Tensor) else len(image)
            all_idx = np.arange(self.num_class)
            if getattr(self, "delete_tag_index", None) is not None:
                valid_idx = np.setdiff1d(all_idx, np.array(self.delete_tag_index, dtype=int))
            else:
                valid_idx = all_idx

            tag_list_np = np.array(self.tag_list)
            tag_list_cn_np = np.array(self.tag_list_chinese)

            k = min(random_k, len(valid_idx))
            tag_output, tag_output_chinese = [], []
            for _ in range(bs):
                chosen = np.random.choice(valid_idx, size=k, replace=False)
                # chosen[:] = 4313
                print('chosen:', chosen)
                tag_output.append(', '.join(map(str, tag_list_np[chosen])))
                tag_output_chinese.append(', '.join(map(str, tag_list_cn_np[chosen])))
