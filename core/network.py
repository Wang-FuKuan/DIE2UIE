...
class DIE(nn.Module):
    ...
    def __init__(self, inp_channels=3, out_channels=3, dim=48, num_blocks=[4, 6, 6, 8],
             ...
             LayerNorm_type='WithBias', decoder=False, **kwargs):
        ...
        self.conv_to_192 = nn.Conv2d(384, 192, kernel_size=1)
        self.prompt_reduce = nn.Conv2d(512, 320, kernel_size=1, bias=False)

        self.dec3_reduce = nn.Conv2d(704, 704, kernel_size=1, bias=False)
        self.dec2_reduce = nn.Conv2d(512, 320, kernel_size=1, bias=False)
        self.dec1_reduce = nn.Conv2d(416, 160, kernel_size=1, bias=False)
    ...
    def forward(self, inp_img, Degradation_feature, Tags_feature):
        ...
        if self.decoder:
            ...
            dec3_param = self.prompt_reduce(dec3_param)
            latent = torch.cat([latent0, dec3_param], 1)
            latent = self.dec3_reduce(latent)
            latent = self.noise_level3(latent)
            latent1 = self.reduce_noise_level3(latent)

            dec3_param = prompt_D.expand(-1, -1, latent0.size(2), latent0.size(3))
            dec3_param = self.prompt_reduce_1(dec3_param)
            latent2 = torch.cat([latent0, latent1, dec3_param], 1)
            latent2 = self.dec3_reduce(latent2)
            latent2 = self.noise_level3(latent2)
            latent0 = self.reduce_noise_level3(latent2)
